        ...
        extends DefaultTraversalVisitor<RelationPlan, Void>
{
    ...
    protected RelationPlan visitJoin(Join node, Void context)
    {
        ...
        if (node.getType() != Join.Type.CROSS && node.getType() != Join.Type.IMPLICIT) {
            ...
            for (int i = 0; i < comparisonTypes.size(); i++) {
                ...
                // hack for implicit conversion when join bigint with varchar
                // add a cast-to-varchar to the bigint side
                Type leftType = symbolAllocator.getTypes().get(leftSymbol);
                Type rightType = symbolAllocator.getTypes().get(rightSymbol);
                if (leftType.equals(BIGINT) && rightType.equals(VarcharType.VARCHAR)) {
                    leftPlanBuilder = rewriteAddBigintToVarcharCast(leftExpressions.get(i), leftPlanBuilder);
                    leftSymbol = leftPlanBuilder.getTranslations().get(leftExpressions.get(i));
                }
                else if (leftType.equals(VarcharType.VARCHAR) && rightType.equals(BIGINT)) {
                    rightPlanBuilder = rewriteAddBigintToVarcharCast(rightExpressions.get(i), rightPlanBuilder);
                    rightSymbol = rightPlanBuilder.getTranslations().get(rightExpressions.get(i));
                }
    ...
    private PlanBuilder rewriteAddBigintToVarcharCast(Expression expression, PlanBuilder planBuilder)
    {
        analysis.addCoercion(expression, VarcharType.VARCHAR);
        TranslationMap translations = new TranslationMap(planBuilder.getRelationPlan(), analysis);
        translations.copyMappingsFrom(planBuilder.getTranslations());
        ImmutableMap.Builder<Symbol, Expression> projections = ImmutableMap.builder();
        for (Symbol symbol : planBuilder.getRoot().getOutputSymbols()) {
            Expression e = new QualifiedNameReference(symbol.toQualifiedName());
            projections.put(symbol, e);
        }

        Map<Symbol, Expression> coerce = coerce(ImmutableList.<Expression>of(expression), planBuilder, translations);
        projections.putAll(coerce);
        return new PlanBuilder(translations, new ProjectNode(idAllocator.getNextId(), planBuilder.getRoot(), projections.build()), planBuilder.getSampleWeight());
    }
